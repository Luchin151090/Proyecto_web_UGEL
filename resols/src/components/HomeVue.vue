  ...
  <v-container>
    <div>
      <v-row>
        ...
        <v-col cols="12">
          <v-card>
            <v-card-title class="pink accent-1 white--text">Últimas 5 Resoluciones</v-card-title>
          <v-data-table
            :headers="headers"
            :items="items2"
            :items-per-page="10"
    ...
    <div>
      <v-btn class="mx-2" color="purple"
      @click="exportpdf">
        <v-icon dark> mdi-download </v-icon>Descargar Informe
      </v-btn>
    </div>
  ...
  data:()=>({
    config_request: {
      ...
      steps:[
        {
            target :'#v-step-0',
            content:'Hola Usuario aquí podrás <strong>observar la información resumida del sistema</strong>'
          },
          {
            target :'#v-step-1',
